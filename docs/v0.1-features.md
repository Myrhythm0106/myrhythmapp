...
Tracked in detail under the **v1.1 Roadmap** section of `strategic-documents/Founding-Member-Launch-Strategy.md`.

- Weekly Anchor digest (G5)
- Formal WCAG 2.2 AA certification (G6 — baseline ships in v0.1)
- Mid-journey persona switcher
- Record-on-behalf backend hardening (Phase 4)
- Support Circle "Record meeting for [name]" full UI

**In v0.1**: friction fixes + a11y baseline (Phase 1), record-on-behalf permission flag (Phase 2, OFF by default), clinical export PDF (Phase 3), GDPR data export (Phase 5).
