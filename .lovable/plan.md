...
| Due in | Read-only, worked out from Finish: "Today", "Tomorrow", "in 4 days", "3 days ago". |
...
Because it is derived, nothing extra is stored and it stays correct without a refresh.

## Technical notes

- `src/components/memoryBridge/ActionsTableView.tsx`: rename the current Due column to Finish, add a Start column reusing the existing `EditableDueDate` component (bound to `start_date`), and add a computed read-only "Due in" cell. New optional prop `onStartDateChange`. Keep the sort control on Finish, add one on Start.
