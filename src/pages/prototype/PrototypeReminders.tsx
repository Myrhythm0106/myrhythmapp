  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        {acts.map(act => (
          <div key={act.id} className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-slate-500 mt-0.5">
                  ...
                  <span className="ml-2 inline-flex items-center gap-1.5 text-slate-600">
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      act.priority === 'high' ? 'bg-slate-900' :
                      act.priority === 'low' ? 'bg-slate-300' : 'bg-slate-500'
                    }`} />
                    {act.priority}
                  </span>
                </div>
              ...
              <button
                ...
                className="text-xs text-slate-500 hover:text-slate-900 flex items-center gap-1 px-2 py-1 rounded-full hover:bg-slate-100"
              >
...
function ReminderRow({ reminder, onRemove, onToggleChannel }: {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white border border-slate-200">
      <BellRing className="w-4 h-4 text-slate-500 flex-shrink-0" />
          ...
          return (
            <button
              ...
              className={`text-[11px] px-2 py-0.5 rounded-full border transition ${
                on ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
              }`}
            >{c.label.toLowerCase()}</button>
          );
        })}
      </div>
      <button onClick={onRemove} className="w-7 h-7 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center" aria-label="Remove">
        <X className="w-4 h-4" />
      </button>
    </div>
  );
...
function AddReminderPicker({ existing, onAdd }: { existing: ReminderOffset[]; onAdd: (o: ReminderOffset) => void }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-slate-100">
      {!open ? (
        <button onClick={() => setOpen(true)} className="text-xs text-slate-700 hover:text-slate-900 font-medium flex items-center gap-1">
