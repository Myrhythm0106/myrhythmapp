...
export function CompactPlanCard({ plan, isSelected, onSelect }: CompactPlanCardProps) {
  ...
  return (
    ...
    >
      {/* Popular Badge - positioned to avoid title overlap */}
      {plan.popular && (
      ...
      {/* Header Section - adjusted for badge space */}
      <div className={cn(
        "flex items-start justify-between mb-3",
        plan.popular && "pr-20" // Add right padding when popular badge is present
      )}>
        ...
        {/* Pricing - only show if no popular badge to avoid cramping */}
        {!plan.popular && (
      ...
      {/* Pricing for popular plans - moved below header */}
      {plan.popular && (
