...
export function UserTypeStep({ onComplete, initialValue }: UserTypeStepProps) {
  ...
  console.log("UserTypeStep: userTypes array:", userTypes);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-8">
      ...
      {/* User Type Cards - Simple and Clear */}
      ...
      {selectedType && (
        <div className="text-center p-4 bg-primary/10 rounded-lg border border-primary/20">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Click "Next" to continue to location setup
          </p>
        </div>
      )}
      ...
      <div className="text-xs text-muted-foreground text-center">
        Debug: {userTypes.length} user types loaded, selected: {selectedType || 'none'}
      </div>
    </div>
  );
}
