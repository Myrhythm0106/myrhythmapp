...
export type UserType = "brain-injury-recovery" | "cognitive-optimization" | "caregiver-support" | "wellness-productivity";
...
export const UserTypeStep = ({
  ...
  const userTypes: UserTypeOption[] = [{
    ...
    isPrimary: true,
    features: ["MYRHYTHM framework assessment", "Empowering goal achievement system", "Wellness monitoring & celebrations", "Cognitive strengthening activities", "Thriving community support"]
  }, {
    ...
    features: ["Performance-focused assessments", "Cognitive enhancement programs", "Advanced brain strengthening", "Productivity optimization", "Achievement insights"]
  }, {
    ...
    features: ["Supportive guidance resources", "Family empowerment tools", "Progress celebration dashboard", "Community connection features", "Empowering educational materials"]
  }, {
    ...
    features: ["Habit mastery tools", "Productivity frameworks", "Wellness optimization", "Goal achievement system", "Life organization mastery"]
  }];
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {userTypes.map(type => <Card key={type.id} onClick={() => handleSelectType(type.id)} className={`cursor-pointer transition-all hover:shadow-md border-2 ${selectedType === type.id ? "border-primary shadow-lg bg-primary/5" : type.isPrimary ? "border-primary/30 hover:border-primary/50 bg-gradient-to-br from-primary/5 to-primary/10" : "border-border hover:border-primary/50"}`}>
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                ...
                {selectedType === type.id && <CheckCircle2 className="h-5 w-5 text-primary flex-shrink-0" />}
              </div>
              {type.badge && <Badge variant="secondary" className="w-fit bg-primary/10 text-primary border-primary/20">
                  ✨ {type.badge}
                </Badge>}
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="space-y-1">
                {type.features.slice(0, 3).map((feature, index) => <div key={index} className="flex items-center gap-2 text-xs text-muted-foreground">
                    <CheckCircle2 className="h-3 w-3 text-primary" />
                    {feature}
                  </div>)}
                {type.features.length > 3 && <div className="text-xs text-muted-foreground">
      ...
      <div className="flex justify-end">
        <Button onClick={handleSubmit} disabled={!selectedType} className="gap-2">
