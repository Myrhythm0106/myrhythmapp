...
import { Calendar, Target, TrendingUp, Crown, Infinity, CheckCircle, ArrowRight, Brain, Heart, Star, Zap } from "lucide-react";
interface LifeEmpowermentGuideProps {
  onComplete: () => void;
  userType?: string;
}
export function LifeEmpowermentGuide({
  onComplete,
  userType
}: LifeEmpowermentGuideProps) {
  ...
  const phases = [{
    ...
    goals: ["Complete your personalized setup", "Create your first daily rhythm", "Explore brain-friendly features", "Connect with your support network"],
    ...
    goals: ["Establish consistent daily routines", "Track your progress and wins", "Refine your rhythm preferences", "Build confidence through small victories"],
    ...
    goals: ["Master advanced planning features", "Optimize your life operating model", "Help others in the community", "Celebrate major milestones"],
    ...
    goals: ["Achieve life-changing milestones", "Become a mentor to others", "Master long-term goal achievement", "Unlock premium life features"],
    ...
    goals: ["Shape your 3-year vision", "Pioneer advanced wellness features", "Lead community transformations", "Create your lasting impact"],
  ...
  return <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-4">
        <div className="flex items-center justify-center gap-2">
          ...
          <h1 className="text-2xl font-bold">Your MyRhythm Timeline</h1>
        </div>
        <p className="text-muted-foreground max-w-2xl mx-auto">See how MyRhythm will evolve with you. This isn't just an app, it's your lifelong partner in personal empowerment and growth.</p>
      </div>

      {/* Timeline Navigation */}
      <div className="flex flex-wrap justify-center gap-2 mb-6">
        {phases.map((phase, index) => <Button key={phase.id} variant={currentPhase === index ? "default" : "outline"} size="sm" onClick={() => setCurrentPhase(index)} className="text-xs">
            {phase.timeframe}
          </Button>)}
      </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {currentPhaseData.goals.map((goal, index) => <div key={index} className="flex items-center gap-2 text-sm">
          ...
          {currentPhase === phases.length - 1 && <div className="bg-gradient-to-r from-violet-100 to-purple-100 p-4 rounded-lg space-y-3">
      ...
      <div className="flex justify-between items-center">
        <Button variant="outline" onClick={handlePrevious} disabled={currentPhase === 0}>
          Previous
        </Button>
        ...
        <Button onClick={handleNext} className="gap-2">
          {currentPhase === phases.length - 1 ? <>Start Your Journey <Heart className="h-4 w-4" /></> : <>Next <ArrowRight className="h-4 w-4" /></>}
        </Button>
      </div>
