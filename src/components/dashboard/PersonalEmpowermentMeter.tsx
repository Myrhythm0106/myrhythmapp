...
interface PersonalEmpowermentMeterProps {
  moodScore: number;
...
export function PersonalEmpowermentMeter({ 
  moodScore, 
  activitiesCompleted, 
  streakDays, 
  goalsProgress 
}: PersonalEmpowermentMeterProps) {
  
  // Calculate overall empowerment score
  ...
  const getEmpowermentLabel = (score: number) => {
    if (score >= 85) return { label: "Thriving & Empowered", color: "text-emerald-600", bg: "bg-emerald-50" };
    if (score >= 70) return { label: "Strong Momentum", color: "text-blue-600", bg: "bg-blue-50" };
    if (score >= 55) return { label: "Building Strength", color: "text-amber-600", bg: "bg-amber-50" };
    return { label: "Growing Power", color: "text-purple-600", bg: "bg-purple-50" };
  };

  const empowermentStatus = getEmpowermentLabel(empowermentScore);
    ...
    <Card className="border-l-4 border-l-purple-400 bg-gradient-to-br from-purple-50/50 to-blue-50/50">
      ...
      <CardContent className="space-y-4">
        <div className="text-center space-y-2">
          ...
          <div className={`inline-flex px-3 py-1 rounded-full ${empowermentStatus.bg}`}>
            <span className={`text-sm font-medium ${empowermentStatus.color}`}>
              {empowermentStatus.label}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>Empowerment Momentum</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-16 h-2 bg-gray-200 rounded-full">
                <div 
        ...
        <div className="pt-2 border-t">
          <div className="text-xs text-center text-gray-600">
            Your empowerment grows stronger each day 🚀
          </div>
        </div>
      </CardContent>
    </Card>
